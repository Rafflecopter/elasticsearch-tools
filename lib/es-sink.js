...
stream
  ...
  .on('end', batch.flush.bind(null, process.exit))


function flushAction(body, cb) {
  client.bulk((baseRequest.body = body) && baseRequest, function (err, resp) {
...
function Batch(batchSize, flushAction) {
  ...
  face.add = function (line) {
    ...
    updateBodyMetaState(line)
  }

  face.needsFlush = function () {
    return !expectSource && totalOperations >= batchSize
  }

  function updateBodyMetaState(line) {
